...
def get_batches_active(train_ind, train_labels, ratio, batch_size=128, shuffle=True):
    """
    Inputs:
        train_ind: np.array
    ...
    train_ind = train_ind[np.random.choice(nums, size=int(ratio*nums), replace=False)]
    nums = train_ind.shape[0]
    if shuffle:
...
def train(train_ind, train_labels, batch_size, train_times, train_probs, ratio):
    ...
    t = time.time()
    model.train()
    prob_thres = np.quantile(train_probs, 1-ratio)
    for epoch in range(train_times):
        for batch_inds, batch_labels in get_batches_active(train_ind, train_labels, ratio, batch_size):
