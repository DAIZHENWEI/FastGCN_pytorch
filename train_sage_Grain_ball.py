...
if __name__ == '__main__':
    ...
    loader = dataloader(args.dataset, args=args)
    train_index, valid_index, test_index, in_feats, labels, n_classes, feats, graph = loader.get_DGL_GCN_inputs()

    """ load train_index selected by GRAIN """
    num_sampled = int(len(train_index) * args.ratio)
    print("Number of training nodes: {}".format(len(train_index)))
    file = '../Grain/examples/sampled_nodes/GRAIN(ball-D)_r0.05_selected_{}_nodes{}.json'.format(args.dataset, num_sampled)

    with open(file) as f:
        train_ind_sampled = json.load(f)   
    train_ind_sampled = torch.from_numpy(np.array(train_ind_sampled))
    print("Number of real picked nodes: {}".format(len(train_ind_sampled)))
    ...
    if args.remove_degree_one:
        np.save(directory + 'GraphSage_accuracy_list_{}_GRAIN_ball_remove_degree_one_ratio{}.npy'.format(args.dataset, args.ratio), test_accs)
    else:
        np.save(directory + 'GraphSage_accuracy_list_{}_GRAIN_ball_ratio{}.npy'.format(args.dataset, args.ratio), test_accs)


    if args.remove_degree_one:
        np.save(directory + 'GraphSage_macro_f1_list_{}_GRAIN_ball_remove_degree_one_ratio{}.npy'.format(args.dataset, args.ratio), test_f1)
    else:
        np.save(directory + 'GraphSage_macro_f1_list_{}_GRAIN_ball_ratio{}.npy'.format(args.dataset, args.ratio), test_f1)