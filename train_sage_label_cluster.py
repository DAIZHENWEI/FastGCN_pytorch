...
def binary_search(cluster_size, ratio, n_sampled):
    cluster_sampled = [max(1, int(size*ratio)) for size in cluster_size]
    ...
    while diff:
        mid = (l+r)/2
        l_sampled = [max(1, int(size*l*ratio)) for size in cluster_size]
        new_sampled = [max(1, int(size*mid*ratio)) for size in cluster_size]
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--fix_ncluster', action='store_true', default=False, help= "fix the number of clusters to 100")
    ...
    if args.fix_ncluster:
        ...
        n_cluster = n_sampled
    kmeans = KMeans(n_clusters=n_cluster, random_state=0).fit(labels_train)
    clusters = kmeans.labels_
    centers = kmeans.cluster_centers_
    cluster_size = [0] * n_cluster
    for i in range(n_cluster):
        cluster_size[i] = sum(clusters==i)
    cluster_sampled = binary_search(cluster_size, args.ratio, n_sampled)
    ...
    for i, center in enumerate(centers):
        ...
        labels_i = labels_train[clusters == i]
        kmeans_i = KMeans(n_clusters=num_per_cluster, random_state=0).fit(labels_i)
        centers_i = kmeans_i.cluster_centers_
        train_index_cluster = train_index[clusters == i]
        for center in centers_i:
            dists = LA.norm(labels_i - center, axis = 1)
    ...
    np.save(directory + 'GraphSage_accuracy_list_{}_label_cluster_ncluster{}_AggregateLayer{}_ratio{}.npy'.format(args.dataset, 
                                                                                                                args.prop_layers, 
                                                                                                                n_cluster,
                                                                                                                args.ratio), test_accs)

    np.save(directory + 'GraphSage_micro_f1_list_{}_label_cluster_ncluster{}_AggregateLayer{}_ratio{}.npy'.format(args.dataset, 
                                                                                                                args.prop_layers, 
                                                                                                                n_cluster,
                                                                                                                args.ratio), test_f1)