...
from utils_sage import load_data, load_pubmed, load_ogbn_arxiv, margin_score
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--warm_start', action='store_true', default=False, help= "use the warm start")
    ...
    """ Randomly re-split the training and testing set """
    train_test_index = torch.cat((train_index, test_index)).numpy()
    num_train = len(train_index)
    train_index = np.random.choice(train_test_index, num_train, replace=False)
    test_index = np.setdiff1d(train_test_index, train_index)
    ...
    # kmeans_test = KMeans(n_clusters=20, random_state=0).fit(feats_test)
    ...
    if epochs < args.epochs and not args.warm_start:
        test_accs, test_f1 = GraphModel.run(epochs=args.epochs-epochs)
        test_accs_list += test_accs
        test_f1_list += test_f1
    ...
    ### Do not use warm start, training from stratch
        GraphModel = GraphSage(args, device, data)
        GraphModel.train_nid = train_ind_sampled
        test_accs, test_f1 = GraphModel.run(epochs=300)
        test_accs_list += test_accs
        test_f1_list += test_f1
        num_train_sampled += [len(train_ind_sampled)] * (args.epochs-epochs)  
    ...
    np.save(directory + 'GraphSage_Shuffle_accuracy_list_{}_batch_at_scale_random_warm_start{}_redundancy{}_pretrain_ratio{}_ratio{}_total_ratio{}.npy'.format(args.dataset, 
                                                                                                            args.warm_start,
                                                                                                            redundancy,
                                                                                                            args.pretrain_ratio,
                                                                                                            args.ratio,
    ...
    np.save(directory + 'GraphSage_Shuffle_macro_f1_{}_batch_at_scale_random_warm_start{}_redundancy{}_pretrain_ratio{}_ratio{}_total_ratio{}.npy'.format(args.dataset, 
                                                                                                            args.warm_start,
                                                                                                            redundancy,
                                                                                                            args.pretrain_ratio,
                                                                                                            args.ratio,
