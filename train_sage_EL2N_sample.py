...
if __name__ == '__main__':
    ...
    loss_fcn_all = nn.CrossEntropyLoss(reduction = 'none')
    ...
    for epochs in range(0, (args.epochs - args.pre_train_epochs) // sample_freq):
        ...
        # EL2N_prob = EL2N/np.sum(EL2N)
        # del pred
        # del EL2N

        loss = loss_fcn_all(y_pred, y_train)
        loss_prob = (loss/torch.sum(loss)).numpy()
        del pred
        del loss

        """ Pick nodes weighted by EL2N """
        num_sampled = int(len(train_index) * args.ratio)
        train_ind_sampled = np.random.choice(train_index, num_sampled, replace=False, p=loss_prob)
        data = train_ind_sampled, valid_index, test_index, in_feats, labels, n_classes, feats, graph
    ...
    np.save(directory + 'GraphSage_accuracy_list_{}_EL2N_sample_pretrain{}_epochs{}_pretrain_ratio{}_ratio{}_sample_freq{}.npy'.format(args.dataset, 
    ...
    np.save(directory + 'Sample_count_list_{}_EL2N_sample_pretrain{}_epochs{}_pretrain_ratio{}_ratio{}_sample_freq{}.npy'.format(args.dataset, 
                                                                                                        args.pre_train_epochs, 
                                                                                                        args.epochs,
                                                                                                        args.pretrain_ratio,
                                                                                                        args.ratio,
